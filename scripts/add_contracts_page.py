#!/usr/bin/env python3
"""Insert ContractsPage before PlaceholderPage and wire into router."""
from pathlib import Path

FILE = Path("/home/z/my-project/src/app/page.tsx")
content = FILE.read_text()

# Insert ContractsPage before PlaceholderPage
insert_marker = "}// ═══════════════════════════════════════════════════════════\n// PLACEHOLDER PAGE"
...
content = content.replace(insert_marker, CONTRACTS_PAGE, 1)

# Wire into router
old_router = '{currentPage === "compliance" && <CompliancePage />}\n        {currentPage === "shipments" && <ShipmentsPage />}'
new_router = '{currentPage === "compliance" && <CompliancePage />}\n        {currentPage === "shipments" && <ShipmentsPage />}\n        {currentPage === "contracts" && <ContractsPage />}'
...
old_exclude = 'currentPage !== "compliance" && currentPage !== "shipments" && ('
new_exclude = 'currentPage !== "compliance" && currentPage !== "shipments" && currentPage !== "contracts" && ('
content = content.replace(old_exclude, new_exclude)

FILE.write_text(content)
print(f"Inserted ContractsPage. New file size: {len(content)} chars")
